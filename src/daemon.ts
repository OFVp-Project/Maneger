...
io.on("connection", (socket) => {
  ...
  socket.on("wireguardServerConfig", () => {
    if (process.env.SHOWDAEMONLOG === "true") console.log("[Daemon]  Requested Wireguard Server Config");
  ...
  socket.on("wireguardUsers", () => {
    if (process.env.SHOWDAEMONLOG === "true") console.log("[Daemon]  Requested Wireguard Users");
  ...
  socket.on("opensshUsers", () => {
    if (process.env.SHOWDAEMONLOG === "true") console.log("[Daemon] Requested OpenSSH Users (Passwords is encrypted)");
